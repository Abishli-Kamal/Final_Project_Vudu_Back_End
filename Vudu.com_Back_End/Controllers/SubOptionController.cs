...
namespace Vudu.com_Back_End.Controllers
{
    ...
    public class SubOptionController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? suboptionId,int id)
        {
            var query = _context.MovieSubOptionSubTitles.AsQueryable();
            if (suboptionId!=null)
            {
                query=query.Where(m => m.SubOptionSubTitle.Id==suboptionId);
            }
